...
class CustomTitleBar(context: Context, attr: AttributeSet? = null) :
    ...
    inline fun initBar(
        ...
        if (centerTextRes != null) {
            centerText.text = context.getString(centerTextRes)
        ...
        centerText.setTextColor(context.colorValue(centerTextColor))
        ...
        //设置标题栏左边的返回图标
        val leftImageView = AlphaImageButton(context)
        leftImageView.scaleType = ImageView.ScaleType.FIT_XY
        leftImageView.setImageDrawable(
            R.drawable.svg_navigation.toDrawable(context, backImageColor)
        )
        leftImageView.setBackgroundResource(R.color.transparent)
        leftImageView.setOnClickListener { activity.finish() }
        ...
        //设置标题栏右边的文字
        val rightText = AlphaTextView(context)
        rightText.setTextColor(context.colorValue(rightTextColor))
        if (rightTextRes != null) {
            rightText.text = context.getString(rightTextRes)
        ...
        //设置标题栏和状态栏的颜色
        try {
            setBackgroundColor(context.colorValue(bgColorRes))
            activity.statusColor(
                context.colorValue(bgColorRes),
                light = isStatusFontDark,
                fitWindow = false
            )
        } catch (e: Exception) {
            e.printStackTrace()
